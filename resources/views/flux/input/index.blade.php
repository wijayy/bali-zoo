...
@php
    ...
    $classes = Flux::classes()
        ->add('w-full block disabled:shadow-none dark:shadow-none')
    ...
    ->add(
        ...
        ->add(
            match (
                ...
                'outline' => $invalid ? 'border-red-500' : 'shadow-xs border-b-2 border-black',
...
<flux:with-field :$attributes :$name>
    <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-flux-input>
        ...
        <input type="{{ $type }}" {{-- Leave file inputs unstyled... --}}
            ...
            @if ($format_number) oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, '.')" @endif
            data-flux-control data-flux-group-target
